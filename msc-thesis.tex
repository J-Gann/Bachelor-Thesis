...
Before the IMP client can access services of the administration portal, it has to be possible to connect the IMP identity with a user profile. The user should be able to connect and disconnect the identity with an existing user profile. As in this scenario the IMP systems only integrate with the administration portal, the user profile should not be modified to include information about an IMP identity.

\begin{center}
    \includegraphics[width=8cm]{Diagrams/Integration 1 Connection Overview.png}
\end{center}

The administration portal can provide an additional web page where a logged in user can connect an IMP identity with his user profile. In order for the web page to initiate a connection process, administration portal and IMP identity require some way to identify each other. Either the IMP identity identifies itself to the administration portal or the administration portal identifies itself to the IMP identity.

The IMP identity could identify itself to the administration portal through manual entering of an IMP ID on the web page. This could enable the administration portal to interact with the identity through a connector. Another option is for the administration portal to identify itself to the IMP identity. As the IMP system provides a functionality for this, it is the better option. Through the connector, the administration portal can create a relationship template. The template is stored on the IMP server but can be accessed by any client which has the correlating template ID. The template contains information for identification of the connector, a encrypted session ID, a reason why the relationship is requested and a list of attributes the IMP client is required to share.

Based on the resulting template ID, the connector creates a QR code which the administration portal includes in web page before sending it to the user. The user can now scan the QR code with the IMP client which retrieves the template corresponding to the template ID from the IMP server. The IMP client displays the request and if the user accepts, the connector is notified of an incoming relationship request. The request contains the template ID, information about the IMP identity of the user, the shared attributes and the encrypted session ID.

The connector transfers the relationship request to the administration portal which based on the decrypted session ID sends a confirmation request to the web page. The confirmation request contains information about the IMP identity which requested to connect to the user profile. If the user accepts the confirmation, the association of the IMP relation and the user ID is stored in a database.

The connection of the user profile and the IMP identity would therefore be an IMP relationship.

\paragraph{Authorisation}

If the user successfully authenticated for a user profile, the administration portal receives a JSON Web Token, which enables it to start actions in the name of the user. The user might want to manage which actions an IMP client is authorised for.

On the web page where the user can connect user profile and IMP identity, authorisation of a connection can be managed too. After a success full connection, the web page presents a UI where the user can determine which actions the IMP client should be able to perform. If the user finishes, the administration portal can request the authorisation from the user profile which requests authentication from a user. The resulting JSON Web Token is then transmitted to the administration portal.

The JSON Web Token contains information to identify the user profile and the authorisations for the entity which possesses this token. It also includes a signature with a private key of the user profile component. This allows the component to check if the token is valid.

This token can be stored in the database where IMP relations are mapped to profile IDs.

\paragraph{Improved Association of IMP ID and OZG ID}

JSON Web Tokens can also function as a connection between user profile and IMP identity because they includes the ID of a user profile. If the IMP client is in possession of this token and requests the administration portal to perform an action, the administration portal can read the profile ID stored inside the token along with the authorisation information. The portal can then verify the validity of the token through the user profile component. If the validation was successful, the portal knows, that whoever sent the request has the authorisation stored in the token for the user profile ID stored in the token.

The JSON Web Token can be saved as attribute of the IMP identity which the administration portal was granted access to. Whenever the portal receives a request from a relation to for example modify an attribute in the user profile, the portal can read the JSON Web Token of the corresponding IMP identity determine the connected user profile ID and verify the authorisation.
...
When the user profile is connected with the IMP identity, an attribute request is issued at least for the JWT issued by the user profile component. In case the JWT authorizes the IMP client to manage personal information of user profile, the administration portal can request access to attributes of the IMP identity which are also stored in the user profile.

As the attributes stored in the user profile might be named differently than the attributes stored in the user profile, some mapping has to take place.

The access of the administration portal to attributes of the IMP identity is stored as part of the relationship.

There are multiple ways to synchronize the attributes. One way would require the administration portal to compare the attributes of the IMP identity with those of the user profile. Another option would be for the IMP client to notify the administration portal of a change in the shared attributes. The second option would be beneficial, as iterative requests to the IMP Client and user profile are resource intensive, especially for a large number of users.

\paragraph{Attribute Mapping}

\paragraph{Inbox}

Both IMP Client and the administration portal provide an inbox functionality. If an IMP identity is connected with a user profile, it can send and receive messages as part of the relation it has with the administration portal. As the administration portal does not store any information on which user profile is connected with which IMP identity, it cannot forward messages of the inbox to the IMP identity of a relation. The IMP client will have to request for new messages from time to time or the previously removed database which maps IMP relations to user profiles is added again.
