...
The following interfaces of system components are relevant:

\paragraph{Administration Portal and Form-Server}
    \begin{itemize}
        \item \textbf{Initiation of an application}
        
            When the user selects to fill in a certain form on an administration portal, the portal submits the following data to the form-server through an interface:
            
            \textbf{JSON Web Token}: 
            If the user authenticated for a user profile on the administration portal, a JWT is created. This token authorizes its owner to automatically access services or resources as the corresponding user profile without the need of additional authentication. In order for the form-server to verify, that the current user is authenticated for the user profile, the portal submits a JWT token to it.
                
            \textbf{User Profile ID}: 
            Each user profile contains a unique identifier. This identifier is submitted to the form server in order to associate the application with a user.
                
            \textbf{Personal Information}: 
            The portal submits personal information to the form-server, so that the form can be automatically filled in. The user is asked if the data should be transmitted beforehand. This is therefore optional.
                
            \textbf{Personal Information Schema ID}: 
            The portal is aware of the currently selected administration service by the user and knows which the corresponding form is. It therefore sends the correct ID of a FIM data field schemata to the form-server. Based on this ID, the form-server knows which form to present to the user.
        
        \item \textbf{Status Update}
        
            When an application was initiated, it is associated with a status. If the status changes, the administration portal gets notified of the new status along with a FMS-ID. The FMS-ID is created by the form server and uniquely identifies the application. The application can have the following states:
            
            \textbf{null}: The application and the corresponding form was successfully created
            
            \textbf{submitted}: The form was filled in and the application is ready to be requested by the administration portal.
            
            \textbf{filed}: The form was transmitted to the administration portal but the portal did not yet verify.
            
            \textbf{confirmed}: The administration portal confirmed the successful reception of the application
            
            \textbf{deleted}: The application was deleted by the form-server.
    
        \item \textbf{Submission of application}
        
            If the user presses a "submit" button on the interactive form, the status of the application changes to submitted and the administration portal is notified. The portal can then request the application data from the from-server through an interface by sending the FMS-ID and receives the following results:
            ...
            This file consists of the personal information the form was filled in with
                
            \textbf{Print version}:
                
            The form-server can optionally sent a printable version of the filled in form
                
            \textbf{Protocol Virus Scan}:
                
            \textbf{Protocol Signature Check}:
        
    \paragraph{Administration Portal and User Profile}
...
\subsection{Basic Use Case Flow Diagram}
...
\begin{itemize}
    \item user wants to manage his data only on one platform
    \item user wants to know who he provisions his personal information
    \item user wants to communicate
    \item user wants to know the reason each time his personal information is requested
\end{itemize}
...
\begin{itemize}
    \item The administration portal / form-server sends the user/app the content of a form. The app will display the form and ask the user to fill it in. The interaction is displayed under a relation between the user and the requesting portal. When submitting, the app sends the data back to the portal and logs that the user transmitted the list of personal data. No authentication is necessary? The content is encrypted and the connector knows the pub. key?
    
    Or relation is made with the responsible administrative institution. The administration portal determines the corresponding institution beforehand. In this case, the communication can also be done through the same relation and the personal data is provisioned to only the institution which needs it, not the whole member state or commune.
    
    The IMP solution has to allow the inititation of relations and after that the request of information for new usage (Verwendungszweck) where the new usage is the application for a administrative service.
\end{itemize}
